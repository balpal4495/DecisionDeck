...
You are a supporting AI in the DecisionDeck codebase. Claude Code is the primary agent — it handles
...
DecisionDeck uses five specialist agents and fifteen skills. For large-context tasks you may be asked to help with:

- **Architect** — traces evidence across Chronicle entries; asks you to survey relevant files before Quorum review
- **DataEngineer** — may ask for schema analysis across `db/schema.ts` and migration files
- **Orchestrator** — may ask you to summarise what phase work has and has not been done
...
## Key invariants to enforce in second-opinion mode

- `oracle.commit()` is never called from application code. Flag any code path that does.
- Importers never auto-save. Every import requires user confirmation before writing.
- Recommendations in reports must cite a source record (decision ID, work item ID, or risk ID).
- Individual engineer performance data must not appear in any feature.
- CSS Modules only — no Tailwind, no inline styles.
- Zod validation at every API boundary and all LLM output.
